 ...
 */
static void udp_csr_set(u8_t epn, u32_t set_bits)
{
	/* UDP is defined in sam4s16.h in the file
	 * z/ext/hal/atmel/asf/sam/include/sam4s/sam4s16c.h
	 */
	volatile Udp * const udp = (volatile Udp * const)UDP;
	uint32_t csr;
 ...
 */
static void udp_csr_clr(u8_t epn, u32_t clr_bits)
{
	volatile Udp * const udp = (volatile Udp * const)UDP;
	u32_t csr;
...
static void udp_read_ep(int epn)
{
	...
	if (epn == 0) {
		...
		for (i = 0; i < total; i++) {
			endpoint0_packet[i] = udp->UDP_FDR[epn];
		}
...
static void udp_ep_isr(int epn)
{
	...
	if (csr & UDP_CSR_TXCOMP) {
		...
		if (epn == 0 && set_address_state == ADDRESS_PENDING_TXCOMP) {
			...
			udp->UDP_FADDR |= UDP_FADDR_FEN | new_device_address;
...
static void udp_reset_event(void)
{
	...
	// __PMC->pcer1 = BIT(2);
	soc_pmc_peripheral_enable(ID_UDP);

	/* Tell the class driver we are disconnected. */

	usb_dc_sam4s.status_cb(USB_DC_DISCONNECTED, NULL);
	...
	udp->UDP_CSR[0] |= UDP_CSR_EPEDS
			   |  UDP_CSR_EPTYPE(UDP_CSR_EPTYPE_CTRL);
	...
	udp->UDP_IER = UDP_IER_WAKEUP | UDP_IER_RXSUSP | 1;
}
...
static void udp_isr(void)
{
	...
	pending_isr = (udp->UDP_ISR & udp->UDP_IMR);
	...
	while (pending_isr) {
		...
		if (pending_isr & UDP_ISR_RXSUSP) {
			...
			udp->UDP_ICR = UDP_ICR_RXSUSP | UDP_ICR_WAKEUP;

			/* Notify the upper layers. */

			usb_dc_sam4s.status_cb(USB_DC_SUSPEND, NULL);
		}
		...
		if (pending_isr & (UDP_ISR_WAKEUP | UDP_ISR_RXRSM)) {
			...
			udp->UDP_ICR = UDP_ICR_WAKEUP | UDP_ICR_RXRSM
				       | UDP_ICR_RXSUSP;
			...
			usb_dc_sam4s.status_cb(USB_DC_RESUME, NULL);
		}
...
int usb_dc_attach(void)
{
	...
	udp->UDP_IER = UDP_IER_RXSUSP | UDP_IER_WAKEUP | UDP_IER_RXRSM
		       | UDP_IER_EXTRSM | UDP_IER_SOFINT;

	/* Connect and enable the UDP's USB interrupt. */

	IRQ_CONNECT(ID_UDP, CONFIG_USB_SAM4S_IRQ_PRIO,
		udp_isr, 0, 0);
	irq_enable(ID_UDP);
...
int usb_dc_ep_set_stall(const uint8_t ep)
{
	...
	udp->UDP_CSR[epn] = UDP_CSR_FORCESTALL;
...
int usb_dc_ep_is_stalled(const uint8_t ep, uint8_t *const stalled)
{
	...
	*stalled = (udp->UDP_CSR[ep] & UDP_CSR_FORCESTALL);
		...
		const uint32_t data_len, uint32_t * const ret_bytes)
{
	...
	while ((udp->UDP_CSR[epn] & UDP_CSR_TXPKTRDY))
		;
		   ...
		   const uint32_t max_data_len, uint32_t *const read_bytes)
{
	...
	len = ((udp->UDP_CSR[epn] & 0xffff0000) >> 16);
	...
	bk0 = (udp->UDP_CSR[epn] & UDP_CSR_RX_DATA_BK0);
	bk1 = (udp->UDP_CSR[epn] & UDP_CSR_RX_DATA_BK1);
			...
			uint32_t *read_bytes)
{
	...
	uint32_t available_data_len = (udp->UDP_CSR[ept] >> 16);
...
int usb_dc_ep_read_continue(uint8_t ep)
{
	...
	if (udp->UDP_CSR[ept] & UDP_CSR_RX_DATA_BK0) {
		udp->UDP_CSR[ept] &= ~UDP_CSR_RX_DATA_BK0;
		SYS_LOG_DBG("EP%d RX_DATA_BK0 cleared.", ept);
	}

	if (udp->UDP_CSR[ept] & UDP_CSR_RXSETUP) {
		udp->UDP_CSR[ept] &= ~UDP_CSR_RXSETUP;
